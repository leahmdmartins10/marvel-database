...
async function getAllSeries(req, res) {
    ...
    if (category === "series") {
        url = `${MARVEL_API_URL}series?ts=${ts}&apikey=${MARVEL_PUBLIC_API_KEY}&hash=${hash}&offset=${offset}&limit=${limit}`;
    } else {
...
async function getAllSeriesByCharacter(req, res) {
    ...
    console.log(`Fetching series for character ID: ${characterId}`);
    ...
    // Marvel API endpoint for series by character
    const url = `${MARVEL_API_URL}characters/${characterId}/series?ts=${ts}&apikey=${MARVEL_PUBLIC_API_KEY}&hash=${hash}&offset=${offset}&limit=${limit}`;
