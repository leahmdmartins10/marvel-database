...
async function getAllEvents(req, res) {
    ...
    if (category === "Events") {
        url = `${MARVEL_API_URL}events?ts=${ts}&apikey=${MARVEL_PUBLIC_API_KEY}&hash=${hash}&offset=${offset}&limit=${limit}`;
    } else {
...
async function getAllEventsByCharacter(req, res) {
    ...
    console.log(`Fetching Events for character ID: ${characterId}`);
    ...
    // Marvel API endpoint for Events by character
    const url = `${MARVEL_API_URL}characters/${characterId}/events?ts=${ts}&apikey=${MARVEL_PUBLIC_API_KEY}&hash=${hash}&offset=${offset}&limit=${limit}`;
